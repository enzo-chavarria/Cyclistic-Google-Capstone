...
![ride length metrics](images/ride_length_metrics.png)

In a provided packet covering the details of this case study, it is stated that Cyclistic users are more
likely to ride for leisure, but about 30% use the bikes to commute to work each day. The data pulled here shows that these commuting users are likely annual members. These members have shorter average ride times but a greater amount of total rides, which is a pattern typical of commuter behavior. 


### Comparing Ride Count by Day of Week and Rider Type

To investigate the relationship of commuters and annual members further, data was pulled to show the ride count for each day of the week for each type of user.
